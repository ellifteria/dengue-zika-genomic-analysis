...
dds <- DESeqDataSetFromMatrix(countData = cts, colData = coldata, design = ~donor) # prepare gene count matrix for DESeq analysis
...
res <- results(dds, contrast=c("donor", "A", "B")) # generate a results table for the DESeq analysis with the contrast specified
res # display the results table

resultsNames(dds) # print the coefficients of the results table

resLFC <- lfcShrink(dds, coef="donor_B_vs_A", type="apeglm") # shrink the effect size for the coefficient `donor_B_vs_A` to facilitate easier visualization and ranking
...
resNorm <- lfcShrink(dds, coef=2, type="normal") # perform shrinkage using the normal method; `coef=2` because we are interested in `donor_B_vs_A`
resAsh <- lfcShrink(dds, coef=2, type="ashr") # perform shrinkage using the adaptive shrinkage estimator: `coef=2` because we are interested in `donor_B_vs_A`
...
plotCounts(dds, gene=which.min(res$padj), intgroup="donor") # plot normalized gene counts by donor

d <- plotCounts(dds, gene=which.min(res$padj), intgroup="donor", returnData=TRUE) # get data from plot of normalized gene counts by donor

library("ggplot2") # load ggplot2 library
ggplot(d, aes(x=donor, y=count)) + 
...
write.csv(as.data.frame(resOrdered),
	  file="donor_B_results.csv") # write the ordered results as a dataframe to a csv file
...
write.csv(as.data.frame(resSig),
	  file="donor_B_results_significant.csv") # write the significant ordered results as a data frame to a csv file
...
resMFinfection_status <- results(ddsMF,
		     contrast=c("infection_status", "ZIKV_infected", "DENV_infected")) # generate results table with contrast specified
...
plotPCA(vsd, intgroup=c("donor", "infection_status")) # perform principal component analysis of vst transform data and plot

pcaData <- plotPCA(vsd, intgroup=c("donor", "infection_status"), returnData=TRUE) # extract data from principal component analysis plot
percentVar <- round(100 * attr(pcaData, "percentVar")) # scale percent variances up to out of 100% instead of out of 1
ggplot(pcaData, aes(PC1, PC2, color=donor, shape=infection_status)) +
	geom_point(size=3) +
