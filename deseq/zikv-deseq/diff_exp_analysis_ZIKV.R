...
gene.counts.file = "./gene-counts-annotations/gene_counts.csv"
gene.annotations.file = "./gene-counts-annotations/gene_annotations_mod.csv"
...
res <- results(dds, contrast=c("infection_status", "ZIKV_infected", "uninfected")) # generate a results table for the DESeq analysis with the contrast specified
res # display the results table

resultsNames(dds) # print the coefficients of the results table

resLFC <- lfcShrink(dds, coef="infection_status_ZIKV_infected_vs_uninfected", type="apeglm") # shrink the effect size for the coefficient `donor_B_vs_A` to facilitate easier visualization and ranking
...
resNorm <- lfcShrink(dds, coef="infection_status_ZIKV_infected_vs_uninfected", type="normal") # perform shrinkage using the normal method; `coef=2` because we are interested in `donor_B_vs_A`
resAsh <- lfcShrink(dds, coef="infection_status_ZIKV_infected_vs_uninfected", type="ashr") # perform shrinkage using the adaptive shrinkage estimator: `coef=2` because we are interested in `donor_B_vs_A`
...
write.csv(as.data.frame(resOrdered),
	  file="./deseq/zikv-deseq/ZIKV_infected_results.csv") # write the ordered results as a dataframe to a csv file
...
write.csv(as.data.frame(resSig),
	  file="./deseq/zikv-deseq/ZIKV_infected_results_significant.csv") # write the significant ordered results as a data frame to a csv file
...
resMFinfection_status <- results(ddsMF,
		     contrast=c("infection_status", "ZIKV_infected", "uninfected")) # generate results table with contrast specified
...
if(file.exists("./Rplots.pdf")){
	file.rename(from="./Rplots.pdf", to="./deseq/zikv-deseq/Rplots.pdf")
}